...
// 프로퍼티와 메서드를 사용할 때는 인스터스명.프로퍼티명 또는 메서드명을 작성한다.
...
console.log(`push() 메소드 : length - ${nums.length} / 배열 - ${nums}`);

let unshift1 = nums.unshift(0);
console.log(`unshift() 메소드 : length - ${nums.length} / 배열 - ${nums}`);

//      pop(), shift() 메서드 : 기존 배열에 맨 끝(앞)에 요소를 1개씩 꺼내는 메서드
let pop1 = chars.pop();
console.log(`pop() 메소드의 반환값 : ${pop1}`)
console.log(`pop() 메소드 : length - ${chars.length} / 배열 - ${chars}`);

let shift1 = chars.shift();
console.log(`shift() 메소드의 반환값 : ${shift1}`);
console.log(`shift() 메소드 : length - ${chars.length} / 배열 - ${chars}`);
